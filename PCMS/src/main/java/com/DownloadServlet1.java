...
public class DownloadServlet1 extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            String sql="select * from prescription where pid=?";
            PreparedStatement ps=con.prepareStatement(sql);
            ps.setString(1,"anshu" );
            ResultSet rs=ps.executeQuery();
            if(rs.next()) {
                java.sql.Blob blob=rs.getBlob("prescription");
                InputStream inp=blob.getBinaryStream();
                int fl=inp.available();
                ServletContext context=getServletContext();
                String mimeType=context.getMimeType("prescription");
                if (mimeType == null) {
                ...
                String headerValue = String.format("attachment; filename=\"%s\"", "prescription");
